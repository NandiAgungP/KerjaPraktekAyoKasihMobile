...
class _SearchPageState extends State<SearchPage> {
  ...
  hapusTransaksi(position) async {
    ...
    try {
      final response = await http.post(
          BaseUrl().hapusPemberi + trans[position]['id_transaksi'].toString(),
          body: {});
      final data = jsonDecode(response.body);
  ...
  dialogConfirm(BuildContext context, position, status) {
    ...
    AlertDialog alert = AlertDialog(
      ...
      content: (status == "1"
          ? Text("Apakah anda yakin ingin memberi barang ini ?")
          : (status == "2"
              ? Text("Apakah anda yakin akan mengirim barang ini ?")
  ...
  getAllItem() async {
    ...
    var id_users = preferences.getInt("id_users");
    final response =
        await http.post(BaseUrl().getTransaksi + id_users.toString(), body: {});
    final data = jsonDecode(response.body);
    this.setState(() {
      trans = data;
      isLoading = false;
